...
//items that are in the cupboard and recipe.
  let inCupboardAndRecipe = pantry['cupboardContents'].filter(x => recipe['ingredients'].includes(x));
///items that are in the fridge and recipe.
  let inFridgeAndRecipe = pantry['fridgeContents'].filter(x => recipe['ingredients'].includes(x));
//items in same array
  let inPantry = inCupboardAndRecipe.concat(inFridgeAndRecipe);
// have only items not in pantry
  let shoppingItems = recipe['ingredients'].filter(x => !inPantry.includes(x));
//create object
  const shoppingList ={};
//add both name and ingredients to object
